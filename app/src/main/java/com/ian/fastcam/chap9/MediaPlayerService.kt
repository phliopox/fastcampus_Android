...
class MediaPlayerService : Service() {
    ...
    override fun onCreate() {
        ...
        val mainPendingIntent = PendingIntent.getActivity(
            baseContext,
            0,
            Intent(baseContext,MusicFragment::class.java).apply {
        ...
        val notification = Notification.Builder(baseContext, CHANNEL_ID)
            ...
            .setContentIntent(mainPendingIntent) // 알림창에 존재하는 notification content 를 클릭했을 때  -> MusicFragment 로 intent
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        when (intent?.action) {
            MEDIA_PLAYER_PLAY -> {
                if (mediaPlayer == null) {
                    mediaPlayer = MediaPlayer.create(baseContext, R.raw.dream_of_octopus)
                }
                mediaPlayer?.start()
            }
            MEDIA_PLAYER_PAUSE -> {
